...
class deviceShadow:
    ...
    # -------------------------------------------------------------- #
    #                   ADDED FOR LEAKSENTINEL                       #
    # -------------------------------------------------------------- #
    def shadowRegisterUpdateCallback(self, srcCallback):
        """
        **Description**

        Listen on update topics for this device shadow by subscribing to update topics. Whenever there
        is an update message generated by the shadow, the registered callback will be called
        and the update payload will be available in the callback.

        **Syntax**

        .. code:: python

          # Listen on update topics for BotShadow
          BotShadow.shadowRegisterUpdateCallback(customCallback)
        ...
        **Returns**
        ...
        with self._dataStructureLock:
            ...
            self._shadowSubscribeCallbackTable["update"] = srcCallback
        # One subscription
        self._shadowManagerHandler.basicShadowSubscribe(self._shadowName, "update", self.generalCallbackIgnoreToken)
        self._logger.info("Subscribed to update topic for deviceShadow: " + self._shadowName)

    def shadowUnregisterUpdateCallback(self):
        """
        **Description**

        Cancel listening on update topics for this device shadow by unsubscribing to update topics. There will
        be no update messages received after this API call.

        **Syntax**

        .. code:: python

          # Cancel listening on update topics for BotShadow
          BotShadow.shadowUnregisterUpdateCallback()
        ...
        **Returns**
        ...
        with self._dataStructureLock:
            # Update callback data structure
            del self._shadowSubscribeCallbackTable["update"]
        # One unsubscription
        self._shadowManagerHandler.basicShadowUnsubscribe(self._shadowName, "update")
        self._logger.info("Unsubscribed to update topics for deviceShadow: " + self._shadowName)

    # special version of general callback allows messages with incorrect tokens to get through
    def generalCallbackIgnoreToken(self, client, userdata, message):
        # In Py3.x, message.payload comes in as a bytes(string)
        # json.loads needs a string input
        with self._dataStructureLock:
            currentTopic = message.topic
                    ...
                    if currentToken is not None:  # and currentToken in self._tokenPool.keys():  # Filter out JSON without the desired token
                        ...
                        if currentType == "accepted":
                            ...
                            # If it is a delete accepted, we need to reset the version
                                ...
                                self._lastVersionInSync = -1  # The version will always be synced for the next incoming delta/GU-accepted response
                        # Cancel the timer and clear the token
                        if currentToken in self._tokenPool.keys():
                            self._tokenPool[currentToken].cancel()
                            del self._tokenPool[currentToken]
                        # Need to unsubscribe?
                        self._shadowSubscribeStatusTable[currentAction] -= 1
